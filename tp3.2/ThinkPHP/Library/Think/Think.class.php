...
class Think {
    ...
    static public function appException($e) {
        ...
        Log::record($error['index'],Log::ERR);
        // 发送404信息
    ...
    static public function halt($error) {
        ...
        if (APP_DEBUG || IS_CLI) {
            ...
            if(IS_CLI){
                exit(iconv('UTF-8','gbk',$e['index']).PHP_EOL.'FILE: '.$e['file'].'('.$e['line'].')'.PHP_EOL.$e['trace']);
            }
        } else {
            //否则定向到错误页面
            ...
            if (!empty($error_page)) {
                ...
                $message        = is_array($error) ? $error['index'] : $error;
                $e['index']   = C('SHOW_ERROR_MSG')? $message : C('ERROR_MESSAGE');
            }
        }
        // 包含异常页面模板
