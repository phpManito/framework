...
<body id="message" onselectstart="return true;" ondragstart="return false;">
	<form action="" method="post" style="text-align:center;margin:10px;">
		查询时间范围:
		<input class="Wdate" name="start_time" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})" autocomplete="off" />
		--
		<input class="Wdate" name="end_time" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})" autocomplete="off" />
					...
					<ul>
						...
						<?php if(is_array($msgs)): foreach($msgs as $key=>$msg): ?><li class="green bounceInDown">
								<h3>
									...
									<?php echo ($msg["uname"]); ?><span><?php echo date('Y-m-d H:i:s',$msg['created_at']);?></span>
									<div class="clr"></div>
								</h3>
								<dl>
